...
#define OSPI_RAM_NODE_ENABLED			DT_NODE_HAS_STATUS(OSPI_RAM_NODE, okay)
#define OSPI_FLASH_NODE_ENABLED			DT_NODE_HAS_STATUS(OSPI_FLASH_NODE, okay)
#define OSPI_EXT_MEM_NODE_ENABLED		(OSPI_RAM_NODE_ENABLED || OSPI_FLASH_NODE_ENABLED)
...
#if OSPI_RAM_NODE_ENABLED
#define OSPI_XIP_NODE			DT_PARENT(OSPI_RAM_NODE)
#if defined(CONFIG_SOC_SERIES_E1) || defined(CONFIG_SOC_SERIES_E1C) || \
	defined(CONFIG_SOC_SERIES_E3) || defined(CONFIG_SOC_SERIES_E5) || \
	defined(CONFIG_SOC_SERIES_E7)
#define OSPI_XIP_MAIR_INDEX		MPU_MAIR_INDEX_DEVICE
#else
#define OSPI_XIP_MAIR_INDEX		MPU_MAIR_INDEX_SRAM
#endif
#define OSPI_XIP_REGION_NAME		"OSPI_RAM_XIP"
#elif OSPI_FLASH_NODE_ENABLED
#define OSPI_XIP_NODE			DT_PARENT(OSPI_FLASH_NODE)
#define OSPI_XIP_MAIR_INDEX		MPU_MAIR_INDEX_FLASH
#define OSPI_XIP_REGION_NAME		"OSPI_FLASH_XIP"
#endif

#define OSPI_XIP_BASE			DT_PROP_BY_IDX(OSPI_XIP_NODE, xip_base_address, 0)
#define OSPI_XIP_SIZE			DT_PROP_BY_IDX(OSPI_XIP_NODE, xip_base_address, 1)
...
#if OSPI_EXT_MEM_NODE_ENABLED
	/* Region 5 */
	MPU_REGION_ENTRY("OSPI_CTRL", ALIF_ENSEMBLE_OSPI_REG,
			 REGION_DEVICE_ATTR(ALIF_ENSEMBLE_OSPI_REG, ALIF_ENSEMBLE_OSPI_SIZE)),

	/* Region 6 */
	MPU_REGION_ENTRY(OSPI_XIP_REGION_NAME, OSPI_XIP_BASE,
			 REGION_OSPI_XIP_ATTR(OSPI_XIP_MAIR_INDEX,
					      OSPI_XIP_BASE, OSPI_XIP_SIZE)),
#endif
};
